...
execute as @s if score @s ing_Core.Trigger_Last matches 633 unless entity @s[tag=ing.Admin] run tellraw @s [{"text":"[Waypoint] ","color":"dark_green"},{"text":"You don't have permission to do this","color":"red","bold":true}]
execute as @s if score @s ing_Core.Trigger_Last matches 633 if entity @s[tag=ing.Admin] run function ing_waypoint:project/script/all/command/delete
