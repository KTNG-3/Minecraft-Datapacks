...
execute as @e[type=minecraft:armor_stand,tag=ing_waypoint.Waypoint] if score @s ing_WP.DATA_Owner = @a[tag=ing_waypoint.LoadPlayer,limit=1] ing_UserData.ID run scoreboard players add @a[tag=ing_waypoint.LoadPlayer,limit=1] ing_WP.LoadPlayer_Slot 1
...
execute as @s if score @s ing_WP.LoadPlayer_Empty matches 1.. at @s if entity @e[type=minecraft:armor_stand,distance=..2] run tellraw @s [{"text":"[Waypoint] ","color":"dark_green"},{"text":"To spawn Waypoint you must be away from Armor Stand at least 2 blocks","color":"red","bold":true}]
