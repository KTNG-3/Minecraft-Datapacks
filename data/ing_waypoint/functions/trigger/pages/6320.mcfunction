...
execute as @s if score @s ing_WP.LoadAll_Empty matches 1.. if entity @s[tag=ing.Admin] at @s if entity @e[type=minecraft:armor_stand,distance=..2] run tellraw @s [{"text":"[Waypoint] ","color":"dark_green"},{"text":"To spawn Waypoint you must be away from Armor Stand at least 2 blocks","color":"red","bold":true}]
