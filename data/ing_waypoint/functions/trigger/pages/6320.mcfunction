...
execute as @s if score @s ing_WP.LoadAll_Empty matches 1.. if entity @s[tag=ing.Admin] at @s if entity @e[type=minecraft:armor_stand,distance=..3] run tellraw @s [{"text":"[Waypoint] ","color":"dark_green"},{"text":"To spawn Waypoint you must be away from other Armor Stand for at least 3 blocks","color":"red","bold":true}]
execute as @s if score @s ing_WP.LoadAll_Empty matches 1.. if entity @s[tag=ing.Admin] at @s unless entity @e[type=minecraft:armor_stand,distance=..3] run function ing_waypoint:project/script/all/command/spawn
