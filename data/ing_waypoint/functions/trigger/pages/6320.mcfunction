...
execute as @e[type=armor_stand] if score @s ing_WP.DATA_Owner matches -1 run scoreboard players add @a[tag=ing_waypoint.All_LoadPlayer,limit=1] ing_WP.LoadAll_Slot 1
...
execute as @s if score @s ing_WP.LoadAll_Empty matches 1.. if entity @s[tag=ing.Admin] at @s if entity @e[type=armor_stand,distance=..2] run tellraw @s [{"text":"[Waypoint] ","color":"dark_green"},{"text":"To spawn Waypoint you must be away from Armor Stand at least 2 blocks","color":"red","bold":true}]
execute as @s if score @s ing_WP.LoadAll_Empty matches 1.. if entity @s[tag=ing.Admin] at @s unless entity @e[type=armor_stand,distance=..2] run function ing_waypoint:project/script/all/command/spawn
