...
execute as @s if score @s ing_WP.DATA_Visibility matches 1 run execute as @a if score @s ing_WP.DATA_ID = @e[tag=ing_waypoint.LoadTarget,limit=1] ing_WP.DATA_ID unless score @e[tag=ing_waypoint.LoadTarget,limit=1] ing_WP.DATA_Owner = @s ing_UserData.ID run function ing_waypoint:project/script/player/command/cancel
